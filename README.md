...
- **Single-day return** on day *t*: $r_t = (P_t - P_{t-1}) / P_{t-1}$
- **Event-window return** from start to end date: $R_{\text{window}} = (P_{\text{end}} - P_{\text{start}}) / P_{\text{start}}$
- **Abnormal return** relative to a benchmark (here XBI): $\text{AR}_{\text{window}} = R_{\text{stock, window}} - R_{\text{benchmark, window}}$ (isolates firm-specific reaction by netting out sector moves)
- **Tone shift**: $\text{tone\_shift} = \text{sentiment}_{\text{Q\&A}} - \text{sentiment}_{\text{prepared}}$ (positive = tone improves under Q&A; negative = tone worsens)
- Optional if available: **earnings surprise**: $\text{Surprise} = \text{EPS}_{\text{actual}} - \text{EPS}_{\text{consensus}}$ (control for beat/miss when testing language effects)
